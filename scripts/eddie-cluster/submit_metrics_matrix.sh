...
submit_metrics() {
  ...
  local checkpoint_stage="${8:-train}"
  ...
  qsub -N "$job_name" -q gpu -l gpu=1 -pe sharedmem 4 -l h_rt="$h_rt" -l h_rss="$h_rss" \
    ...
    -v PROJECT_DIR="$PROJECT_DIR",EDDIE_SCRATCH_ROOT="$EDDIE_SCRATCH_ROOT",DATASET_LABEL="$dataset_label",MODEL_NAME="$model_name",CONFIG_PATH="$config_path",CLASSIFIER_CONFIG="$classifier_config",BATCH_SIZE="$batch_size",CHECKPOINT_STAGE="$checkpoint_stage" \
    "$SCRIPT"
}
...
submit_metrics morphomnist hvae counterfactual_benchmark/methods/deepscm/configs/morphomnist/hvae.json counterfactual_benchmark/methods/deepscm/configs/morphomnist/classifier.json 16:00:00 16G 128 hvae_finetune
...
submit_metrics celeba_simple hvae counterfactual_benchmark/methods/deepscm/configs/celeba/simple/hvae.json counterfactual_benchmark/methods/deepscm/configs/celeba/simple/classifier.json 36:00:00 24G 64 hvae_finetune
...
submit_metrics celeba_complex hvae counterfactual_benchmark/methods/deepscm/configs/celeba/complex/hvae.json counterfactual_benchmark/methods/deepscm/configs/celeba/complex/classifier.json 36:00:00 24G 64 hvae_finetune
